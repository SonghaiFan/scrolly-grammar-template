...
export function inferTransition(previous: SpecLike, next: SpecLike): string[] {
  ...
  const granularity = !layoutOnly && (
    diff.hasDelta('granularity') || diff.hasDelta('bar.granularity') || aggregateChanged
  );
  ...
  if (diff.hasDelta('filter') || diff.hasDelta('focus')) scenes.push('focus');
  ...
  if (fieldChanged && !swapped && !granularity) scenes.push('observation');
  if (granularity) scenes.push('granularity');
  if (swapped || coordinatesChanged || layoutOnly || diff.hasDelta('guide') || diff.hasDelta('bar.guide')) scenes.push('guide');
  return scenes;
}
