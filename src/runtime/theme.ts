...
const stylesheets = new Map<string, StylesheetLease>();

/** Inline variables belong to a target; external stylesheets remain document-wide. */
export async function applyTheme(theme: AnyRecord = {}, target: Element) {
  ...
  const dispose = () => {
    ...
    previous.forEach(({ value, priority, applied }, name) => {
      ...
      if (style.getPropertyValue(name) !== applied || style.getPropertyPriority(name)) return;
...
function acquireStylesheet(href: string) {
  const key = new URL(href, document.baseURI).href;
  let entry = stylesheets.get(key);
  if (!entry) {
    const existing = Array.from(document.querySelectorAll<HTMLLinkElement>('link[rel~="stylesheet"]')).find(link => link.href === key);
    const link = existing ?? document.createElement('link');
    let cancel = () => {};
    const ready = existing?.sheet ? Promise.resolve() : new Promise<void>((resolve, reject) => {
      let settled = false;
      ...
      const loaded = () => finish();
      const failed = () => finish(new Error(`ScrollyLite theme stylesheet failed to load: ${href}`));
  ...
  return {
    ...
    release() {
      if (released) return;
      released = true;
      if (--retained.users) return;
      retained.cancel();
      if (retained.owned) retained.link.remove();
      if (stylesheets.get(key) === retained) stylesheets.delete(key);
    }
  };
}
...
function themeVariables(theme: AnyRecord = {}) {
  ...
  for (let i = 1; i <= 10; i++) aliases[`color-series-${i}`] = [`series-${i}`];
  for (const [source, targets] of Object.entries(aliases)) {
    if (!(('--sl-' + source) in variables)) continue;
    for (const target of targets) if (!(('--sl-' + target) in variables)) variables['--sl-' + target] = variables['--sl-' + source];
  }
  return variables;
}
