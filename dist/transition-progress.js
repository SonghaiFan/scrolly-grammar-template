...
function collectSchedules(nodes, { transitionId, transitionName } = {}) {
    ...
    nodes.forEach((node) => {
        ...
        Object.entries(schedules).forEach(([id, schedule]) => {
            if (transitionId != null && Number(id) !== Number(transitionId))
                return;
            if (transitionName != null && schedule.name !== transitionName)
                return;
            schedule.timer?.stop();
            items.push({
                id, node, schedule,
...
function compilePropertyTracks(items, minTime, span) {
    ...
    const sample = (elapsed) => {
        for (const propertyChange of propertyChanges) {
            ...
            for (let i = 1; i < propertyChange.segments.length && propertyChange.segments[i].start <= elapsed; i++)
                segment = propertyChange.segments[i];
