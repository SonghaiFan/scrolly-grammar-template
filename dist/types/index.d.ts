...
export type Renderer<S extends ViewSpec = ViewSpec> = (chart: ChartContext, rows: DataRow[], spec: S, tooltip: TooltipContext, d3: D3Lib) => void;
...
export type ActionType = 'enter' | 'exit' | 'step' | 'scroll' | 'progress' | 'tooltip' | 'input' | 'scrub' | 'click' | 'unclick' | (string & {});
export type Direction = 'up' | 'down' | (string & {});
export type ActionToken = 'step' | 'scroll' | 'tooltip' | 'enter' | (string & {});
export type RawActionEvent = number | string | Event | {
    type?: string;
    step?: number;
    index?: number;
    value?: number;
