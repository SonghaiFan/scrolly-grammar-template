...
function fitRadius(chart, requestedRadius, { columns = 1, rows = 1 } = {}) {
    return Math.max(2, Math.min(requestedRadius, chart.innerWidth / Math.max(columns * 2.45, 1), chart.innerHeight / Math.max(rows * 2.45, 1)));
}
