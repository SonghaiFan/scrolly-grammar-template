...
export function createBarRenderKit(deps) {
    ...
    function steps(chart, rendererOrientation, d3) {
        ...
        if (!plan?.steps?.length)
            return null;
        if (plan.target?.renderer !== rendererOrientation)
            return null;
        const ordered = Array.isArray(plan.steps)
            ? plan.steps.filter((step) => (step.part === 'x' || step.part === 'y') &&
    ...
    function axisTransition(steps, part, d3) {
        if (!steps?.ordered?.length)
            return null;
        const index = steps.ordered.findIndex((step) => step.part === part && step.changes.includes('axis'));
        if (index < 0)
            return null;
    ...
    function renderBarJoin(options) {
        const { chart, rows, spec, tooltip, d3, bindTooltip, key, category, className, orientation, rx = 3, fill, geometry, startGeometry = geometry?.start, targetGeometry = geometry?.target, steps, markGeometry = geometry ? { x: geometry.applyX, y: geometry.applyY } : undefined, applyGeometry = geometry?.apply, exitGeometry = geometry?.exit } = options;
        chart.g.selectAll('rect.sl-bar')
            ...
            const prepared = update
                ...
                .call(bindTooltip, spec, tooltip);
            if (steps) {
                return applyMarkSteps(prepared, steps, spec, markGeometry, (selection) => selection.style('opacity', (d) => barSelectionOpacity(d, spec, themeValue('--sl-dim-opacity', 0.22))).attr('fill', fill));
            }
            return prepared
                .transition(chart.transition.base)
                .delay((d, i) => staggerDelay(spec, d, i))
