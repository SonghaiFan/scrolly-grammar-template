...
export function getScene(node, viewConfig, d3) {
    if (node.__visDeltaScene)
        return node.__visDeltaScene;
...
function applyGuideScene(chart, rows, spec) {
    ...
    layer.selectAll('circle.sl-guide-dot')
        ...
        .join((enter) => enter.append('circle').attr('class', 'sl-guide-dot')
        .attr('cx', (d) => d.x).attr('cy', (d) => d.y).attr('r', 0)
        .transition(chart.transition.base).attr('r', 5), (update) => update.transition(chart.transition.base).attr('cx', (d) => d.x).attr('cy', (d) => d.y).attr('r', 5), (exit) => exit.transition(chart.transition.base).attr('r', 0).remove());
}
function joinGuideLine(layer, className, data, transition, attrs) {
    ...
    layer.selectAll(`line.${className}`)
        ...
        .join((enter) => setAttrs(enter.append('line').attr('class', `sl-guide-rule ${className}`))
        .style('opacity', 0).transition(transition).style('opacity', 1), (update) => setAttrs(update.transition(transition)).style('opacity', 1), (exit) => exit.transition(transition).style('opacity', 0).remove());
}
