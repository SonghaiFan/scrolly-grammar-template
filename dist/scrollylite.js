// @ts-nocheck — Story composition and legacy embedding runtime.
import { applyTransforms } from './data/transforms.js';
import { chartModules } from './charts/manifest.js';
import { defaultScrollProgress, hasScrollAction, normalizeActionEvent, normalizeActionTokens } from './runtime/actions.js';
import { createChartRuntimeDeps } from './runtime/chart-deps.js';
import { snapshotChartRegistry } from './runtime/chart-registry.js';
...
import { installTransitionProgress } from './transition-progress.js';
import { clearSceneTransitionProgress, SCROLL_TRANSITION_NAME } from './transition-progress.js';
import { renderChartShell } from './runtime/chart-shell.js';
import { createViewRenderer } from './runtime/view-renderer.js';
import { createTransitionSurface as mountTransitionSurface } from './runtime/transition-surface.js';
import { chartRegistry as BUILT_IN_CHART_IDIOMS, registerChartIdiom, registerChartModule, availableChartIdioms } from './runtime/chart-registry.js';
export { registerChartIdiom, registerChartModule, availableChartIdioms };
export function createTransitionSurface(from, to, options) {
    return mountTransitionSurface(from, to, options, BUILT_IN_CHART_IDIOMS);
}
export async function createStory(spec, options) {
    ...
    try {
        ...
        return {
            ...
            to(index) { renderer.action({ type: 'click', step: index, action: ['step', 'tooltip'], force: true }); },
            destroy
        };
    }
    catch (error) {
...
export async function createChart(spec, options) {
    ...
    if (!Number.isSafeInteger(requestedStep))
        throw new Error('initialStep must be a finite integer.');
    const data = await loadData(compiled.data, runtime.d3);
    const mount = await prepareMount(target, compiled, data, runtime);
    let renderer;
    let destroyed = false;
...
function createRenderer(shell, spec, datasets, runtime) {
    ...
    const action = (event, options = {}) => {
        if (destroyed)
            throw new Error('This chart runtime has been destroyed.');
    ...
    return {
        ...
        destroy() {
            ...
            if (shell.tooltip)
                shell.tooltip.style.opacity = '0';
            if (runtime.context)
                runtime.context.colors = null;
        }
    };
}
...
function resolveThemePalette(root) {
    if (typeof document === 'undefined')
        return [];
