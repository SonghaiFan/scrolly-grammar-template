...
import { createViewCompiler, dash, diffViewStates, escapeHtml, serializeViewSpec } from 'visdelta/composition';
...
export function renderShell(target, spec, options = {}) {
    ...
    <div class="sl-intro-inner">
      <p class="sl-kicker">Scrollytelling</p>
...
function stepCompiledViewSpec(step = {}) {
    ...
    if (!viewSpec?.mark || viewSpec.mark === 'text')
        return null;
...
function summarizeDelta(delta = {}) {
    return { type: delta.type, action: delta.action, previous: compactDiffValue(delta.previous), next: compactDiffValue(delta.next) };
}
function compactTransitionPlan(plan = {}) {
    const compact = { ...plan };
    delete compact.diff;
    if (plan.match)
        compact.match = plan.match;
    if (plan.enter)
        compact.enter = plan.enter;
    if (plan.exit)
        compact.exit = plan.exit;
    return compact;
}
function compactDiffValue(value) {
    ...
    return { keys: Object.keys(value), summary: text.slice(0, 320) };
}
